...
**Known-bad pattern, already hit once**: after #182's `alineo`/agent naming inversion, the docs update (`5dcb3de`) edited the existing `v0.1` content folder *in place* instead of cutting a real `v0.2` — reasonable at the time (nothing had shipped under the new names yet), but once #190 published `0.2.0` to npm, `v0.1` was silently describing `0.2.0`'s API with no real old-version snapshot anywhere except git history. Fixed in `docs/dynamic-version-registry` (PR #193) by moving that content to `v0.2` and restoring the true pre-rename docs (from `5dcb3de~1`) as `v0.1`. **If a future breaking-change PR touches `content/docs/core/v0.1/` or `content/docs/alineo/v0.1/` directly instead of adding a new version folder, stop and check whether that's actually a version cut being done wrong.**
