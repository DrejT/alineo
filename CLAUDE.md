...
**Two rules from this:**
1. **A feature PR that adds/changes documented `core` or CLI API and is going out in the next release should cut the new `vX.Y` folder itself** (`cp -r` the latest, edit the copy, `bun run build`), not edit the current latest in place.
2. **After any `chore: version packages` merge that bumps `@alineo-labs/core` or `alineo-cli` by minor (pre-1.0) — check whether a docs version cut is owed** and hasn't happened. If `content/docs/core/vX.Y/` still matches the *previous* published version, cut the new folder.
