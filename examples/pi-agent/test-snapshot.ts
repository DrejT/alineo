/**
 * Alineo snapshotting test — verifies that a second Alineo.start() for the same
...
const t1 = Date.now();
const agent1 = await Alineo.start(spec, { adapter });
...
const t2 = Date.now();
const agent2 = await Alineo.start(spec, { adapter });
