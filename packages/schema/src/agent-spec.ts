/**
 * `AgentSpec` — the JSON an agent is defined by.
 *
 * Both the documented `interface` and the `AgentSpecSchema` that validates it are kept by
 * hand, deliberately: the interface carries doc comments that `z.infer<>` would erase, and
 * they are what a reader sees on hover. `agent-spec.test.ts` asserts at the type level that
 * the two cannot drift apart, which buys the safety of a single source without the DX loss.
 *
 * `validateAgentSpec()` and `AgentSpecValidationError` stay in `@alineo-labs/agent` — they
 * are behaviour, and this package has none.
 *
 * Moved here from `@alineo-labs/agent` so that `apps/alineod` can stop modelling it as
 * `z.record(z.unknown())` and emit a real `AgentSpec` into its OpenAPI spec.
 */
