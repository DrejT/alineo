 ...
 * -- shared by every test file that stubs `globalThis.fetch` instead of a cast at each call
 * site. Still a real `Mock` (assertable via `expect(stubFetch(...)).toHaveBeenCalled()` etc.),
 * just also assignable directly to `globalThis.fetch` with no cast needed at the assignment
 * site. */
