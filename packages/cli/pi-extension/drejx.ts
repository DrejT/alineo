 ...
 * ensureDrejxReady() bootstrap below still work) — no removal planned yet.
 *
 * Used to also register drejx_spawn/drejx_prompt/drejx_agents/drejx_kill as typed tool calls,
 * wrapping the CLI's session-lifecycle primitives so a Pi agent didn't have to hand-roll shell
 * commands for them. Removed (see issue #21 Bug B): `drejx fork` — the core RLM fan-out
 ...
 * typed `drejx_spawn` tool over the `drejx fork` shell command the guidance text recommended for
