 ...
 * unlike `alineo fork`, which forks a running session's *live* sandbox state
...
export async function spawn(
  ...
  if (!specPath)
    throw new Error(
      "Usage: alineo spawn <spec> [--prompt <msg>] [--rebuild] [--depth N] [--max N] " +
...
export const spawnCommand: CliCommand = {
  ...
  group: "agent",
  variants: [
    { usage: "alineo spawn <spec>", summary: "Start a fresh agent sandbox, print its name, exit" },
    {
