...
`alineo fork` is meant to be run **by a session's own Pi bash tool**, not from a fresh host shell — `name` labels the _caller's own_ running session, resolved to a sandbox ID two ways:
...
The caller's sandbox is then resolved via [`Alineo.attach()`](/docs/reference/agent-sdk#agentattach) — **not** `Alineo.resume()`, which would kill the very bridge process making this call — and [`agent.spawn(childSpec, { spawnDepth, maxAgents })`](/docs/reference/agent-sdk#agentspawn) forks its live sandbox into the child.
...
- See the [Pi extension](/docs/reference/cli#pi-extension) section on the commands index for how a Pi session gets `alineo fork` syntax injected into its own guidance automatically.
