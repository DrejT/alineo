...
Connects to the target with [`Alineo.reattach()`](/docs/reference/agent-sdk#alineoreattach) and calls
[`agent.steer(message)`](/docs/reference/agent-sdk#agentsteer).

`reattach()` binds to the bridge already running in the target sandbox **without restarting it** — unlike
[`alineo prompt`](/docs/reference/cli/prompt), which uses `Alineo.resume()` and would abort a turn in progress.
...
it's doing, use [`alineo kill`](/docs/reference/cli/kill).
...
The [Pi extension](/docs/reference/cli#pi-extension) includes this guidance in the system prompt of sessions
...
- Sandboxes are addressed by ID, not name, for the same reasons as [alineo prompt](/docs/reference/cli/prompt#why-sandbox-id-not-name).
- For steering agents managed by the [alineod](/docs/guide/swarms) daemon, use its
  [`POST /agents/:id/steer`](/docs/reference/http-api#post-agentsagentidsteer) route instead.
