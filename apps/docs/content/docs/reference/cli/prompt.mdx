...
Calls [`Alineo.resume(sandboxId, { adapter, specPath })`](/docs/reference/agent-sdk#agentresume), sends `message`, and prints the full reply once Pi finishes responding. `Alineo.resume()` restarts the sandbox's bridge process — Pi and the workspace are untouched, but any interactive stream a _different_ process had open against that sandbox will be interrupted.
