...
On first load the container is checkpointed after install. Subsequent calls restore from the snapshot — skipping the install and starting in ~3s instead of ~90s. See [Snapshotting](/docs/guide/agents/snapshots).
...
`onEgressRequest` is **required** when the spec has any `env` credential binding with `approval: "hold"` — it decides each first outbound request to a held host. See [Permission gate — holding network egress](/docs/guide/control/permissions#holding-network-egress-for-approval).
...
Reconnect to an existing sandbox **without restarting its bridge**. Where `resume()` kills and restarts the bridge process — dropping any turn in progress — `reattach()` only looks up the sandbox's proxy URL and checks that the bridge answers. The returned `Alineo` is fully functional: `.prompt()`, `.bash()`, and `.steer()` continue the same Pi process and conversation.

Use it when your own process restarted but the sandbox didn't — a control plane coming back up after a crash (this is how [alineod recovers a swarm](/docs/guide/durability/crash-recovery)), or a CLI steering an agent that is busy mid-turn ([`alineo steer`](/docs/reference/cli/steer)).

It **fails fast (within 5 s) if the bridge doesn't answer** — a container can be running while the bridge inside it has died. Fall back to `resume()` in that case. `spec`/`specPath` resolve the same way as for `resume()`.
...
`onPermission` auto-resolves each `permission_request` on the stream with the handler's decision, instead of calling `resolvePermission()` by hand — the events still flow through the stream. Only meaningful when the spec sets `permissions`. See [Permission gate](/docs/guide/control/permissions).
...
The message is delivered after the tool calls in the current turn finish and before Pi's next model call — it does not interrupt a tool call that is already running. Use `abort()` to stop immediately.
...
Active only when `AgentSpec.permissions` is set to something other than `"auto"`. See [Permission gate](/docs/guide/control/permissions) for the full model.
...
Direct access to the underlying `SandboxHandle` — the full [Core SDK Sandbox API](/docs/guide/sandboxes), bypassing Pi. Use this to read or write files, run shell commands, or inspect the container independently of Pi.
...
| `permissions`          | `"auto" \| "ask" \| "readonly" \| PermissionPolicy?` | Human-in-the-loop tool-call gate. Omit (or `"auto"`) for the current behavior — no gate. See [Permission gate](/docs/guide/control/permissions).                                                                                                                                                                                                                                                                                                                               |
...
See [Workspace setup](/docs/guide/agents/workspace-setup) for details and examples.
...
See [Streaming & tool events](/docs/guide/agents/streaming) for usage examples.
