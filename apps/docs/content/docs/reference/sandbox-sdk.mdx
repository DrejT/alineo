...
| `credentialBroker` | `CredentialBroker` | No       | Backend for `sb.credentials.*`. Defaults to `OpenSandboxCredentialBroker` (`@alineo-labs/vault`) — see [Credentials](/docs/guide/control/credentials). |
...
See [Sandboxes](/docs/guide/sandboxes) for `SandboxOptions` reference.
...
4. Re-registers any credentials the sandbox had bound (see [Credentials](/docs/guide/control/credentials)) — `opts.resolveCredential` supplies values that can't be re-derived automatically
...
This is also what `sb.fork()` uses under the hood — see [Forking sandboxes](/docs/guide/swarms/forking) for the relationship between `checkpoint()`, `fork()`, `resume()`, and `restoreSnapshot()`.
...
Register, update, remove, and list credentials injected into this sandbox's outbound requests — requires the sandbox to have been created with `credentialProxy: true`. See [Credentials](/docs/guide/control/credentials).
