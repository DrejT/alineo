...
See [File operations](/docs/guide/sandboxes/file-ops) for full examples.
...
Use `client.resume(sandboxId)` to restore from the last checkpoint. See [Snapshots & replay](/docs/guide/sandboxes/snapshots).
...
See [Control flow](/docs/guide/sandboxes/control-flow) for full docs.
