...
`alineo add` only fetches and saves an `AgentSpec` JSON file — it never creates a sandbox, runs setup, or checkpoints anything. To actually run the agent programmatically, load the spec with [`alineo`](/docs/reference/agent-sdk)'s `Alineo.load()`, as shown below. If you just want to run it from the shell without writing TypeScript, use [`alineo spawn`](/docs/reference/cli/spawn) instead — it wraps the same `Alineo.load()` call.
...
See [Snapshotting](/docs/guide/agents/snapshots) for the full details.
...
- [Agent SDK reference](/docs/reference/agent-sdk) — the full `Alineo` API: prompting, streaming, sessions, snapshotting
- [alineo core SDK — exec](/docs/guide/sandboxes/exec)
- [alineo core SDK — file operations](/docs/guide/sandboxes/file-ops)
