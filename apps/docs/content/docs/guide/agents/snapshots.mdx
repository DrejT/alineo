...
On the first call to `Alineo.load()`, the SDK installs the Pi CLI inside a `node:22` sandbox, runs any [workspace setup steps](/docs/guide/agents/workspace-setup), then checkpoints the container. On every subsequent call it restores from that snapshot — skipping the install entirely.
