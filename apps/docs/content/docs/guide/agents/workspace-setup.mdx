...
See [Credentials](/docs/guide/control/credentials) for what injection means and how it behaves across `resume()`/`fork()`/spawned children.
...
Any change to a step's `name`, `run`, or `cwd` is included in the [setup hash](/docs/guide/agents/snapshots) and automatically invalidates the snapshot. The next `Alineo.load()` will run all steps again from scratch and create a new checkpoint.
