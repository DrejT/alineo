...
A child is created with the SDK's [`agent.spawn()`](/docs/reference/agent-sdk#agentspawn): the parent's
...
see [Crash recovery](/docs/guide/durability/crash-recovery).
...
[event stream](/docs/reference/events).
...
[crash recovery](/docs/guide/durability/crash-recovery) possible. Deleting a run releases its sandboxes but keeps its
ledger rows.
