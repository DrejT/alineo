...
- A running OpenSandbox server — `bunx alineo-cli init` is the quickest way (see [alineo init](/docs/reference/cli/init)).
...
A run starts with one root agent. The body's `spec` is an ordinary [agent spec](/docs/reference/agent-sdk);
...
See [Events](/docs/reference/events) for every event type.
...
- [Fan-out and gather](/docs/guide/swarms/fan-out-gather) — `waitFor`, input injection, and idempotent spawns.
- [Steering and pausing](/docs/guide/agents/steering) — redirect or freeze agents mid-run.
- [Crash recovery](/docs/guide/durability/crash-recovery) — what happens when alineod restarts under a live swarm.
