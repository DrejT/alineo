...
`networkPolicy` (which hosts are reachable) and `credentialProxy` (transparent credential injection) are separate concerns that ride the same sidecar. `credentialProxy: true` **requires** `networkPolicy` to also be set. See [Credentials](/docs/guide/control/credentials).

For an agent that should hold a specific host behind a **human decision** before it's reachable, see [Permission gate](/docs/guide/control/permissions#holding-network-egress-for-approval).
