...
Credential injection rides on the same opt-in egress layer as [network policy](/docs/guide/control/egress) — pass `credentialProxy: true` when creating the sandbox:
