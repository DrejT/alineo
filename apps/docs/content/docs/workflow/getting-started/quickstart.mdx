...
```bash
npm install alineo @alineo-labs/workflow
```
...
No `close()` call needed either — `Alineo` has no such method; the adapter closes itself automatically when the process exits.
