...
This only saves the spec locally — it doesn't run anything. See [Running an agent spec](/docs/alineo/using-sandboxes) for how to actually load and run it with `Alineo.start()`.

## Minimal example

Every spec runs inside a `node:22` sandbox (there's no `image` field — the base image is fixed) and requires `harness: "pi"`:
