...
description: Load an agent spec saved by alineo add with @alineo-labs/agent's Agent.load() — alineo add itself never creates a sandbox.
---

`alineo add` only fetches and saves an `AgentSpec` JSON file — it never creates a sandbox, runs setup, or checkpoints anything. To actually run the agent programmatically, load the spec with [`@alineo-labs/agent`](/docs/agent)'s `Agent.load()`, as shown below. If you just want to run it from the shell without writing TypeScript, use [`alineo spawn`](/docs/alineo/v0.1/commands/spawn) instead — it wraps the same `Agent.load()` call.

## Setup

```bash
bun add @alineo-labs/agent @alineo-labs/sqlite
```
...
`agent.sandbox` is the live `Sandbox` from `@alineo-labs/core` — use it for anything not covered by the `Agent` API, like reading files or exposing a port:
