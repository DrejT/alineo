...
Load it with: Agent.load("agents/code-reviewer.json") from @alineo-labs/agent
```
...
`Agent.load()` (from `@alineo-labs/agent`) does the actual sandbox work: on first load it spins up a `node:22` sandbox, installs the Pi CLI and any `setup` steps, then checkpoints it. Subsequent loads restore from that snapshot, skipping the install.
...
const agent = await Agent.load("agents/code-reviewer.json", { adapter });
...
`Agent.load()` reads `alineo.config.json` itself for the OpenSandbox server URL and other defaults — the same file `alineo init` wrote in step 1. You don't need to construct a `Alineo` client or pass connection details by hand — the one thing you still construct yourself is the storage adapter (`opts.adapter`), since `@alineo-labs/agent` doesn't depend on any specific one.
...
- [Commands reference](/docs/alineo/v0.1/commands) — all flags for each command
- [Registry format](/docs/alineo/v0.1/registry) — publish your own agent spec
- [Running an agent spec](/docs/alineo/v0.1/using-sandboxes) — full details on `Agent.load()`
- [@alineo-labs/agent overview](/docs/agent) — the full Agent API (prompting, streaming, sessions, snapshotting)
