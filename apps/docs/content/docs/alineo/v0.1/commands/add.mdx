...
`add` never creates a sandbox, runs `setup`, or contacts the OpenSandbox server directly — it's a local file operation. The sandbox work happens later, the first time you call `Agent.load()` on the saved spec (see [Running an agent spec](/docs/alineo/v0.1/using-sandboxes)).
...
Load it with: Agent.load("agents/node-toolchain.json") from @alineo-labs/agent
```
