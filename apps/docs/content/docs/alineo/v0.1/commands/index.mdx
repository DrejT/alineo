...
These wrap `@alineo-labs/agent`'s `Agent.load()`/`Agent.resume()`/`Agent.attach()`/`Agent.spawn()` directly. Sessions are usually addressed by **sandbox ID** — `prompt` and `kill` require it (see [alineo prompt](/docs/alineo/v0.1/commands/prompt#why-sandbox-id-not-name) for why). `fork` and `logs` are addressed by session **name** instead — see their own pages for why. `spawn` starts a fresh session and takes a spec path, not an identifier.

<Cards>
  <Card
    href="/docs/alineo/v0.1/commands/spawn"
    title="spawn"
    description="Start a fresh, independent agent sandbox from a spec's own snapshot."
  />
  <Card
    href="/docs/alineo/v0.1/commands/prompt"
    title="prompt"
    description="Send one prompt to a running sandbox and print the reply."
  />
  <Card
    href="/docs/alineo/v0.1/commands/fork"
    title="fork"
    description="Fork a running session's own live sandbox into a new child agent."
  />
  <Card
    href="/docs/alineo/v0.1/commands/agents"
    title="agents"
    description="List running agent sessions, cross-checked against the live control plane."
  />
  <Card href="/docs/alineo/v0.1/commands/kill" title="kill" description="Stop a sandbox by ID." />
  <Card
    href="/docs/alineo/v0.1/commands/logs"
    title="logs"
...
See [alineo fork](/docs/alineo/v0.1/commands/fork) for the full mechanics.
