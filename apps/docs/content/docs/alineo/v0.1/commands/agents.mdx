...
3. Also lists sandboxes actually running on the same OpenSandbox server that **aren't** in this CLI invocation's own ledger at all — labeled "Untracked". This is the normal case for a child sandbox created via `alineo fork` from inside another sandbox, since that fork uses the child sandbox's _own_ internal ledger, not the host's.
