...
description: Load an agent spec saved by alineo add with Alineo.start() — alineo add itself never creates a sandbox.
---

`alineo add` only fetches and saves an `AgentSpec` JSON file — it never creates a sandbox, runs setup, or checkpoints anything. To actually run the agent programmatically, load the spec with [`alineo`](/docs/agent)'s `Alineo.start()`, as shown below. If you just want to run it from the shell without writing TypeScript, use [`alineo start`](/docs/alineo/commands/start) instead — it wraps the same `Alineo.start()` call.
...
const agent = await Alineo.start("agents/node-toolchain.json", { adapter });
...
- **Subsequent loads**: restore from that checkpoint, skipping the install entirely — much faster. The snapshot is keyed on a hash of `harness`, `harnessVersion`, `packages`, and `setup`, so changing any of those forces a fresh install on the next load.
- Pass `{ rebuild: true }` to force a full reinstall regardless of the cache: `Alineo.start(specPath, { adapter, rebuild: true })`.
...
```ts
const agent = await Alineo.start("agents/web-service.json", { adapter });
...
Each `Alineo.start()` call for the same spec restores an independent sandbox from the same checkpoint — running one doesn't affect another:

```ts
const [a1, a2] = await Promise.all([
  Alineo.start("agents/reviewer.json", { adapter }),
  Alineo.start("agents/reviewer.json", { adapter }),
]);
...
- `Alineo.start()` creates a sandbox (or resumes one). It costs resources until `agent.close()` is called.
- Always call `agent.close()` when done — use `try/finally` to ensure it runs on error.
- The checkpoint from the first load is preserved. You can `Alineo.start()` the same spec as many times as needed without repeating setup.
