...
`add` never creates a sandbox, runs `setup`, or contacts the OpenSandbox server directly — it's a local file operation. The sandbox work happens later, the first time you call `Alineo.load()` on the saved spec (see [Running an agent spec](/docs/alineo/0.3/using-sandboxes)).
