...
Written once, in the project root (not under `.alineo/`). Contains the connection details and defaults used by both `alineo` and `@alineo-labs/agent`.
...
| `useServerProxy`     | Always `true` when started via `alineo init`. `@alineo-labs/agent` reads this for its `Alineo` client. |
