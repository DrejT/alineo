...
3. Also lists sandboxes actually running on the same OpenSandbox server that **aren't** in this CLI invocation's own ledger at all — labeled "Untracked". This is the normal case for a child sandbox created via `alineo spawn` from inside another sandbox, since that fork uses the child sandbox's _own_ internal ledger, not the host's.
...
| `SANDBOX ID` | The full sandbox ID — pass this to `alineo prompt`, `alineo stop`, `alineo logs`.                                                                                                                                                                        |
...
- If nothing is running, prints `(no running alineo-tracked sessions — run 'alineo start <spec>' to start one)` instead of an empty table.
