...
2. Fetches the URL (or reads the local file) and validates it as an `AgentSpec`: `name` must be a string, `harness` must be `"pi"`. All other fields are unchecked.
...
`add` never creates a sandbox, runs `setup`, or contacts the OpenSandbox server directly — it's a local file operation. The sandbox work happens later, the first time you call `Alineo.start()` on the saved spec (see [Running an agent spec](/docs/alineo/using-sandboxes)).
...
Load it with: Alineo.start("agents/node-toolchain.json") from alineo
```
