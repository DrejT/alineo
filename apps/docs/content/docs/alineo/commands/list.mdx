...
| `HARNESS`     | The spec's `harness` field (currently always `"pi"`). Truncated to 7 characters.                      |
...
- `list` only reflects what's in `agentsDir` on disk — a spec appearing here does not mean it has ever been `Alineo.start()`ed or has a sandbox snapshot yet.
