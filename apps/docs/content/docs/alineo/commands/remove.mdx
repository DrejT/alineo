...
- `remove` only deletes the spec JSON file. It does not delete any sandbox snapshot created by a prior `Alineo.start()` — that's tracked separately, in `agent-snapshots.json` next to your ledger — nor does it touch the ledger itself.
