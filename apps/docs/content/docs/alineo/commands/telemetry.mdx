...
| `specProvider`                                             | `"nvidia"` — `start`/`spawn` only, read from the target spec's own `provider` field |
...
```bash
ALINEO_TELEMETRY_DISABLED=1 alineo start ./agents/my-agent.json
# or the cross-tool convention:
DO_NOT_TRACK=1 alineo start ./agents/my-agent.json
```
