...
**Session 1** — loads `agents/support-agent.json` via `Alineo.start()`, wired with a `Memory`
...
**Session 2** — calls `Alineo.start()` again with the **same spec** and the **same `Memory`
