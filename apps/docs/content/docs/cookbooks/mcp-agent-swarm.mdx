...
[`alineo-mcp`](/docs/alineod/mcp)'s tools instead of raw `alineod` HTTP calls — the MCP client counterpart, useful for
...
<Callout title="Where to go next">
  [`alineo-mcp`](/docs/alineod/mcp) covers every tool and how to configure it in Claude Code, Claude
  Desktop, or Cursor; [Fan-out and gather](/docs/alineod/concepts/fan-out-gather) and [Steering and
  pausing](/docs/agent/concepts/steering) cover the mechanics used here; the [HTTP
