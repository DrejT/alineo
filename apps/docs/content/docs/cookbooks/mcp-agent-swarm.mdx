...
`run_start` loads `agents/lead.json` (which installs `git` and clones `expressjs/cors` as a setup step)
...
Two `agent_spawn` calls fork the lead's live sandbox into reviewers for security and correctness. Each
...
`agent_pause` / `agent_resume` freeze and thaw the correctness reviewer's container for five
seconds without restarting it; `agent_steer` narrows the security reviewer's brief mid-turn — the steer
...
One more `agent_spawn` call spawns the editor with `waitFor` on both reviewers. alineod holds it until
...
The merged report comes back from `result_get`, is printed and written to `review.md`, followed by the
final agent tree from `run_get`. `run_stop` then closes every sandbox in the swarm while keeping
the run's history.

</Step>
</Steps>

Progress for every agent streams via repeated `run_watch` calls, each a bounded 20-second window
