...
<CookbookMeta
  difficulty="Beginner"
  time="~5 min"
  primitives={["exec", "resources", "timeouts"]}
/>
...
<Callout title="Reach for this pattern when...">
  ...
  script, a plugin. Swap `runUntrusted()`'s body for `execCode()`/`createCodeContext()` (see
  [Code Interpreter](/docs/examples/exec-code)) if you want a stateful REPL instead of one-shot
  scripts.
</Callout>
