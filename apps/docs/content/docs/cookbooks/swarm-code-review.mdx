...
A code review run by a swarm of agents, orchestrated entirely through [alineod](/docs/guide/swarms)'s HTTP API. One lead
...
<Callout title="Where to go next">
  [Fan-out and gather](/docs/guide/swarms/fan-out-gather) and [Steering and
  pausing](/docs/guide/agents/steering) cover the mechanics used here, and the [HTTP
  API](/docs/reference/http-api) lists every route.
</Callout>
