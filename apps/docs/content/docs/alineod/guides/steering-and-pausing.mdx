...
- Resume always returns the agent to `running`.

<Callout type="warn" title="Long pauses and the inactivity timeout">
  A paused turn still counts toward `ALINEOD_PROMPT_INACTIVITY_MS` (default 180 s). If a pause
  outlasts it, the turn is ended shortly after resume and settles with whatever it had produced.
  Raise the timeout if you expect long pauses mid-turn.
</Callout>
