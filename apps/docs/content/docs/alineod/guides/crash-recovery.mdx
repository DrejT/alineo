...
The `agents` and `handles` tables are discarded and rebuilt by replaying the ledger. Any agent in
`provisioning`, `spawning`, `running`, or `paused` is considered live and needs reconnecting.
...
which restarts the bridge (dropping any in-flight turn). If that fails too, the agent ends as `lost`.

Parents whose own turn already finished are reconnected as well when a pending child still needs them as its
fork source.
