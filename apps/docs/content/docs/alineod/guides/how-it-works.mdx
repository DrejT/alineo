...
| State          | Meaning                                                                  |
| -------------- | ------------------------------------------------------------------------ |
| `provisioning` | Accepted; the sandbox is being created or forked.                        |
...
A finished turn does not close the agent's sandbox. An agent in `done` can still be prompted again or used as a
spawn parent.
...
- If a turn stalls (no stream activity for `ALINEOD_PROMPT_INACTIVITY_MS`), it is ended; any partial text it had
  produced still becomes the result, with outcome `success`.
