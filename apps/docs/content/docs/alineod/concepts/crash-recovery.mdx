...
For each one, alineod calls [`Alineo.reattach()`](/docs/agent/api-reference/alineo#alineoreattach), which binds to
...
If the bridge doesn't answer, alineod falls back to [`Alineo.resume()`](/docs/agent/api-reference/alineo#alineoresume),
