...
A child is created with the SDK's [`agent.spawn()`](/docs/agent/api-reference/alineo#agentspawn): the parent's
...
see [Crash recovery](/docs/alineod/concepts/crash-recovery).
...
[crash recovery](/docs/alineod/concepts/crash-recovery) possible. Deleting a run releases its sandboxes but keeps its
ledger rows.
