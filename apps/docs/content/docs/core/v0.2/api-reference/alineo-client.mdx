...
| `credentialBroker` | `CredentialBroker` | No       | Backend for `sb.credentials.*`. Defaults to `OpenSandboxCredentialBroker` (`@alineo-labs/vault`) — see [Credentials](/docs/core/0.2/concepts/credentials). |
...
See [Sandboxes](/docs/core/0.2/concepts/sandboxes) for `SandboxOptions` reference.
...
4. Re-registers any credentials the sandbox had bound (see [Credentials](/docs/core/0.2/concepts/credentials)) — `opts.resolveCredential` supplies values that can't be re-derived automatically
...
This is also what `sb.fork()` uses under the hood — see [Forking sandboxes](/docs/core/0.2/patterns/fork) for the relationship between `checkpoint()`, `fork()`, `resume()`, and `restoreSnapshot()`.
...
Register, update, remove, and list credentials injected into this sandbox's outbound requests — requires the sandbox to have been created with `credentialProxy: true`. See [Credentials](/docs/core/0.2/concepts/credentials).
