...
See [File operations](/docs/core/0.2/building/file-ops) for full examples.
...
Use `client.resume(sandboxId)` to restore from the last checkpoint. See [Snapshots & replay](/docs/core/0.2/building/snapshots).
...
See [Control flow](/docs/core/0.2/building/control-flow) for full docs.
