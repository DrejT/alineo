...
2. Writes a `sandbox.checkpoint_created` event to the ledger (same as `sb.checkpoint()`)
...
| Writes `sandbox.checkpoint_created` to ledger | Yes               | Yes                         |
...
Because `fork()` writes a `sandbox.checkpoint_created` event, the resulting snapshot is visible in `sb.listCheckpoints()` and can also be used with `client.resume()`.
