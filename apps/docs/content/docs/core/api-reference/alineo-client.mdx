...
Creates a container, waits until it reaches `Running` state, and returns a live `SandboxHandle` object. Writes a `sandbox.created` event to the ledger.
...
1. Reads the ledger to find the last `sandbox.checkpoint_created` event (or the last one matching `opts.tag`)
