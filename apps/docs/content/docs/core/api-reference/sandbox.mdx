...
This is also what `sb.fork()` uses under the hood — see [Forking sandboxes](/docs/alineod/concepts/forking) for the relationship between `checkpoint()`, `fork()`, `resume()`, and `restoreSnapshot()`.
