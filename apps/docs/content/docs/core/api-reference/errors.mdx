---
title: Errors
description: CommandError, SandboxError, ExecConnectionError, WorkflowError, and AlineoError — the full alineo error hierarchy.
---
...
└── AlineoError                — client-level errors from Alineo itself (not a WorkflowError)
```

`SandboxError`, `ExecConnectionError`, and `CommandError` all extend `WorkflowError`, which extends `Error` — catch all three with `e instanceof WorkflowError`. `AlineoError` is a separate, sibling class thrown by `Alineo` client methods (`sandbox()`, `resume()`, `sandboxes.*`, `environment()`) rather than by `Sandbox` methods — see [AlineoError](#alineoerror) below.
...
```ts
import { WorkflowError } from "alineo";
...
A failure during `client.sandbox()`'s own initial creation throws `AlineoError`, not `SandboxError` — see below.

```ts
import { SandboxError } from "alineo";
...
```ts
import { ExecConnectionError } from "alineo";
...
Thrown by the `Alineo` client for API-level errors (404 not found, timeouts waiting for Running state):

```ts
import { AlineoError } from "alineo";

try {
  await client.resume("nonexistent-id");
} catch (e) {
  if (e instanceof AlineoError) {
...
`AlineoError` is not a `WorkflowError` — it's a separate class for client-level errors.
