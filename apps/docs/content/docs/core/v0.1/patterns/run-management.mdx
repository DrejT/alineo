...
```ts
import { SandboxStatus } from "alineo";
...
`client.resume()` throws `AlineoError` (status 404) if no session with that ID is found in the ledger, and if no checkpoint exists in the session's history.
