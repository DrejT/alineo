...
Each forked sandbox acquires a concurrency slot (counts against `AlineoOptions.maxConcurrency`). The original sandbox holds its slot as well. If you fork N times from one sandbox, you need N+1 available slots.
