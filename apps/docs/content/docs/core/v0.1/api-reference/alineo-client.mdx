...
```ts
new Alineo(options: AlineoOptions)
```
...
See [Sandboxes](/docs/core/v0.1/concepts/sandboxes) for `SandboxOptions` reference.
...
Throws `AlineoError` (404) if the session is not found or has no matching checkpoint.
...
This is also what `sb.fork()` uses under the hood — see [Forking sandboxes](/docs/core/v0.1/patterns/fork) for the relationship between `checkpoint()`, `fork()`, `resume()`, and `restoreSnapshot()`.
...
Attaches to an already-`Running` sandbox by ID, no snapshot or ledger replay involved — the container must already be running (throws `AlineoError` (409) otherwise). Use this to reconnect to a live sandbox created outside the current process, e.g. from a saved `sandboxId`.
