...
See [Sandboxes](/docs/core/0.1/concepts/sandboxes) for `SandboxOptions` reference.
...
This is also what `sb.fork()` uses under the hood — see [Forking sandboxes](/docs/core/0.1/patterns/fork) for the relationship between `checkpoint()`, `fork()`, `resume()`, and `restoreSnapshot()`.
