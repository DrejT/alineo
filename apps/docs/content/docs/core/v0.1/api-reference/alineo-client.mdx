...
```ts
await client.resume(sandboxId: string, opts?: { tag?: string }): Promise<SandboxHandle>
```
...
4. Returns a `SandboxHandle` — pre-checkpoint execs return cached results; later execs run live

Throws `SandboxClientError` (404) if the session is not found or has no matching checkpoint.
...
| `sb.fork(tag?)`                   | `Promise<SandboxHandle>`              | Snapshot and return an independent copy               |
