...
Import `CodeLanguage` from `alineo`:
...
See [File operations](/docs/core/v0.1/building/file-ops) for full examples.
...
Use `client.resume(sandboxId)` to restore from the last checkpoint. See [Snapshots & replay](/docs/core/v0.1/building/snapshots).

## Control flow (workflow builder)

The `@alineo-labs/workflow` package adds queued control-flow steps: `retry`, `when`, `forEach`. These are only available in the workflow builder (`SandboxBuilder`), not on the `Sandbox` class directly.
...
See [Control flow](/docs/core/v0.1/building/control-flow) for full docs.
