...
An **environment** is a named sandbox configuration with a setup recipe. alineo builds it once — installs packages, writes config, compiles assets — snapshots the result, and restores from that snapshot on every subsequent call. Setup never runs again unless you explicitly rebuild.
