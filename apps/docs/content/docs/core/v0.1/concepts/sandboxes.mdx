...
| `hooks`     | `SandboxHooks`                                  | Lifecycle callbacks for observability. See [Observability](/docs/core/v0.1/patterns/observability).                                              |
...
`AlineoOptions.maxConcurrency` caps how many sandboxes may be active at once. When at capacity, `client.sandbox()` awaits until a slot is free (released by `sb.close()`):
