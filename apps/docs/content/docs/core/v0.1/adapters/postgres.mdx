...
For local development and single-process scripts, `@alineo-labs/sqlite` is simpler — no database server required. See [SQLite adapter](/docs/core/0.1/adapters/sqlite).
