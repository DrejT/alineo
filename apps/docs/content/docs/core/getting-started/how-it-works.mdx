...
```
exec.started    { cmd, seq }
exec.output    { seq, type: "stdout", text: "..." }  ← one per chunk
exec.completed { seq, exitCode }
```

`client.sandbox()` writes `sandbox.created`. `sb.close()` writes `sandbox.closed`.

This means every run is fully replayable from the ledger — even if the process crashes mid-exec.

## Checkpoint and resume

`sb.checkpoint()` calls the OpenSandbox snapshot API, waits for the snapshot to become ready, and writes a `sandbox.checkpoint_created` event with the snapshot ID.
