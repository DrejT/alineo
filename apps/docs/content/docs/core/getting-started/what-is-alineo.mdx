...
description: alineo gives you live sandbox containers as first-class objects — spawn, exec, checkpoint, resume.
---

alineo is a sandbox execution substrate built on top of [OpenSandbox](https://opensandbox.ai). You call `client.sandbox()` to get a live container, call methods on it, and close it when done. Every exec is durably logged to a ledger — if a run is interrupted, `client.resume()` restores from the last checkpoint and replays prior execs from cache.
