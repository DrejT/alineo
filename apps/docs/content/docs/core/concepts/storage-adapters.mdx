...
| Event                        | When                                                      |
| ---------------------------- | --------------------------------------------------------- |
| `sandbox.created`            | Container created and reached Running state               |
| `exec.started`               | `sb.exec()` or `sb.execCode()` called                     |
| `exec.output`                | Stdout/stderr chunk received from execd                   |
| `exec.completed`             | exec finished; payload has `{ seq, exitCode }`            |
| `sandbox.checkpoint_created` | `sb.checkpoint()` completed; payload has `{ snapshotId }` |
| `sandbox.closed`             | `sb.close()` called                                       |
