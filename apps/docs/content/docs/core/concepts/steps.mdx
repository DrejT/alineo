...
See [File operations](/docs/core/concepts/file-ops) for full examples.
...
Use `client.resume(sandboxId)` to restore from the last checkpoint. See [Snapshots & replay](/docs/core/concepts/snapshots).
...
See [Control flow](/docs/core/concepts/control-flow) for full docs.
