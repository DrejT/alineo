---
title: Snapshotting
description: How Alineo.start() caches the Pi install so subsequent loads take seconds instead of minutes.
---

On the first call to `Alineo.start()`, the SDK installs the Pi CLI inside a `node:22` sandbox, runs any [workspace setup steps](/docs/agent/getting-started/workspace-setup), then checkpoints the container. On every subsequent call it restores from that snapshot — skipping the install entirely.
...
- `harness` — currently always `"pi"`
- `harnessVersion` — pinned version or `"latest"`
- `packages` — APT packages sorted alphabetically
- `setup` — workspace setup steps (names, commands, working directories)

Any change to these fields automatically invalidates the snapshot and triggers a full rebuild on the next `Alineo.start()` call. Fields that don't affect the snapshot — `model`, `provider`, `env`, `resources` — never invalidate the cache.
...
```ts
const agent = await Alineo.start(spec, { adapter, rebuild: true });
```
...
If the snapshot no longer exists on the OpenSandbox server (e.g. after a server restart), `Alineo.start()` detects the failure, logs a `warn` (`snapshot restore failed, rebuilding`, with the underlying `reason`), and falls back to a full install automatically.
