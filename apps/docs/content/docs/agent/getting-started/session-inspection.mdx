...
`agent.getBranchPoints()` lists the points in the current session a conversation can be branched from:
...
if (points.length > 0) {
  const { text, cancelled } = await agent.branchSession(points[0].entryId);
  console.log(`Branched at: "${text.slice(0, 60)}" (cancelled: ${cancelled})`);
}
```

Each entry corresponds to a user turn in the harness's session history. Branching creates a new session branch at that point — useful for trying alternative responses or replaying from a known state. It is a conversation branch, not a sandbox copy: `agent.sandbox.fork()` is what does the latter.
