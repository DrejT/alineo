...
Pi has built-in transient-error recovery that fires automatically when the AI provider returns a 429 (rate limit) or 5xx (server error). The `@alineo-labs/agent` bridge surfaces this as observable events so you can build responsive UIs without writing retry logic yourself.
