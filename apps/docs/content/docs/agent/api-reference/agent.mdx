...
Load and validate the JSON spec at `specPath`, spin up a `node:22` sandbox, install the Pi CLI and any `setup` steps, and return a ready `Alineo`.
...
```ts
static async resume(sandboxId: string, opts: { adapter: IStorageAdapter; specPath?: string }): Promise<Alineo>
```
...
Direct access to the underlying `SandboxHandle` — the full [Core SDK Sandbox API](/docs/core/v0.1/concepts/sandboxes), bypassing Pi. Use this to read or write files, run shell commands, or inspect the container independently of Pi.
