...
Use it when your own process restarted but the sandbox didn't — a control plane coming back up after a crash (this is how [alineod recovers a swarm](/docs/alineod/guides/crash-recovery)), or a CLI steering an agent that is busy mid-turn ([`alineo steer`](/docs/alineo/commands/steer)).
...
Fork **this agent's own live sandbox** — filesystem, installed packages, checked-out state, everything currently on disk — into a brand-new independent sandbox running its own Pi bridge. Unlike `Alineo.start()` (always starts from a spec's own snapshot) or `agent.branchSession()`/`agent.duplicateSession()` (harness conversation branching — same container, same bridge, new session branch), this is sandbox-level forking: the child sees exactly what this agent's sandbox sees right now, including uncommitted work. No install/setup steps run — the child inherits whatever is already installed on this agent's sandbox.
...
Branch the harness conversation at its current position, creating a new session file. Same container, same bridge — nothing on disk is copied. Returns whether the branch was cancelled.
...
Branch the harness conversation from a specific message entry in its history. `entryId` comes from `getBranchPoints()` (see below), not `getMessages()` — `PiMessage` has no `id`/`entryId` field. Returns the text of the message branched from, and whether it was cancelled.

Both of these branch a **conversation**. `agent.sandbox.fork()` is the one that copies a filesystem; these never leave the container.

```ts
const points = await agent.getBranchPoints();
if (points.length > 0) {
  const branched = await agent.branchSession(points[0].entryId);
  console.log(`Branched from: "${branched.text.slice(0, 60)}"`);
}
```
...
The JSON shape of an agent spec file. Pass the path to `Alineo.start(specPath)`.
...
| `harness`              | `"pi"`                                               | Agent-loop driver to run. Only `"pi"` is supported.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `harnessVersion`       | `string?`                                            | npm version specifier for the Pi CLI (e.g. `"1.2.3"`, `"^1.2.0"`, or a dist-tag like `"latest"`). Passed to `npm install -g @earendil-works/pi-coding-agent@<harnessVersion>`. Omit to install whatever npm resolves as latest. Included in the setup-hash cache key, so changing it forces a fresh Pi CLI install.                                                                                                                                                                    |
| `model`                | `string`                                             | **Required.** Model ID passed to Pi via `--model`.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
