...
On first load the container is checkpointed after install. Subsequent calls restore from the snapshot — skipping the install and starting in ~3s instead of ~90s. See [Snapshotting](/docs/agent/concepts/snapshots).
...
`onEgressRequest` is **required** when the spec has any `env` credential binding with `approval: "hold"` — it decides each first outbound request to a held host. See [Permission gate — holding network egress](/docs/agent/concepts/permissions#holding-network-egress-for-approval).
...
Use it when your own process restarted but the sandbox didn't — a control plane coming back up after a crash (this is how [alineod recovers a swarm](/docs/alineod/concepts/crash-recovery)), or a CLI steering an agent that is busy mid-turn ([`alineo steer`](/docs/cli/commands/steer)).
...
`onPermission` auto-resolves each `permission_request` on the stream with the handler's decision, instead of calling `resolvePermission()` by hand — the events still flow through the stream. Only meaningful when the spec sets `permissions`. See [Permission gate](/docs/agent/concepts/permissions).
...
Active only when `AgentSpec.permissions` is set to something other than `"auto"`. See [Permission gate](/docs/agent/concepts/permissions) for the full model.
...
| `permissions`          | `"auto" \| "ask" \| "readonly" \| PermissionPolicy?` | Human-in-the-loop tool-call gate. Omit (or `"auto"`) for the current behavior — no gate. See [Permission gate](/docs/agent/concepts/permissions).                                                                                                                                                                                                                                                                                                                               |
...
See [Workspace setup](/docs/agent/concepts/workspace-setup) for details and examples.
...
See [Streaming & tool events](/docs/agent/concepts/streaming) for usage examples.
