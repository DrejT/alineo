...
Any change to a step's `name`, `run`, or `cwd` is included in the [setup hash](/docs/agent/concepts/snapshots) and automatically invalidates the snapshot. The next `Alineo.load()` will run all steps again from scratch and create a new checkpoint.
