...
`alineo add` only fetches and saves an `AgentSpec` JSON file — it never creates a sandbox, runs setup, or checkpoints anything. To actually run the agent programmatically, load the spec with [`alineo`](/docs/agent/api-reference/alineo)'s `Alineo.load()`, as shown below. If you just want to run it from the shell without writing TypeScript, use [`alineo spawn`](/docs/cli/commands/spawn) instead — it wraps the same `Alineo.load()` call.
...
See [Snapshotting](/docs/agent/concepts/snapshots) for the full details.
...
- [Agent SDK reference](/docs/agent/api-reference/alineo) — the full `Alineo` API: prompting, streaming, sessions, snapshotting
- [alineo core SDK — exec](/docs/core/concepts/exec)
- [alineo core SDK — file operations](/docs/core/concepts/file-ops)
