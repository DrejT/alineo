...
`contextUsage.percent` tells you how full Pi's context window is. Pi will auto-compact when it approaches the limit (see [Streaming — Compaction events](/docs/agent/concepts/streaming#compaction-events)), but you can also monitor it proactively:
