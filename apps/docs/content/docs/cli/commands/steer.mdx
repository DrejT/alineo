...
Connects to the target with [`Alineo.reattach()`](/docs/agent/api-reference/alineo#alineoreattach) and calls
[`agent.steer(message)`](/docs/agent/api-reference/alineo#agentsteer).

`reattach()` binds to the bridge already running in the target sandbox **without restarting it** — unlike
[`alineo prompt`](/docs/cli/commands/prompt), which uses `Alineo.resume()` and would abort a turn in progress.
...
it's doing, use [`alineo kill`](/docs/cli/commands/kill).
...
The [Pi extension](/docs/cli/commands#pi-extension) includes this guidance in the system prompt of sessions
...
- Sandboxes are addressed by ID, not name, for the same reasons as [alineo prompt](/docs/cli/commands/prompt#why-sandbox-id-not-name).
