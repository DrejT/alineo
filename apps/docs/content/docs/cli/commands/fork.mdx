...
The caller's sandbox is then resolved via [`Alineo.attach()`](/docs/agent/api-reference/alineo#agentattach) — **not** `Alineo.resume()`, which would kill the very bridge process making this call — and [`agent.spawn(childSpec, { spawnDepth, maxAgents })`](/docs/agent/api-reference/alineo#agentspawn) forks its live sandbox into the child.
...
- See the [Pi extension](/docs/cli/commands#pi-extension) section on the commands index for how a Pi session gets `alineo fork` syntax injected into its own guidance automatically.
