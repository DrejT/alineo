...
An alineo "registry item" is just a URL that returns an `AgentSpec` JSON object (the same spec type used by [`alineo`](/docs/agent/api-reference/alineo)). Any URL works — a GitHub raw file, a Gist, your own server, or the small curated set of examples alineo hosts at [registry.alineo.tech](https://registry.alineo.tech) — there's no requirement to publish through a central service.

<Cards>
  <Card
    href="/docs/cli/registry/schema"
...
This only saves the spec locally — it doesn't run anything. See [Running an agent spec](/docs/agent/concepts/spec) for how to actually load and run it with `Alineo.load()`.
