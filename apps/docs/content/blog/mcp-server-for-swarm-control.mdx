...
| `result_get`                              | `GET /agents/:agentId/result`   | Resolve a settled agent's output                        |
| `init`                                    | —                               | Start OpenSandbox + alineod locally via Docker          |
| `spec_add` / `list_specs` / `remove_spec` | —                               | Manage the local saved-agent-spec cache                 |
...
`result_get`'s `waitSeconds`, `run_watch`'s `maxWaitSeconds`, a
`waitFor`-carrying `agent_spawn` — needs to outlast the _client's_ own request timeout,
...
That includes the bootstrap tool. `init` starts OpenSandbox and alineod locally via
...
`init` now detect that and fall back to bridge networking automatically — one fix, both
...
`init` to start one), same as any alineod client. And it isn't on npm yet — build it from
