...
coverAlt: A terminal panel titled AgentSpec.permissions showing four tool calls resolving to allow, ask, or deny.
---
...
```json
{ "name": "my-agent", "harness": "pi", "permissions": "readonly" }
```
...
`ask`, the extension holds the call and the SDK emits a `permission.requested` on the agent
...
The gate runs **inside** the Pi process. It stops a misbehaving _model_ — it is not a barrier
