...
[`AgentSpec.permissions`](/docs/agent/getting-started/permissions) gate pausing a risky tool
call for a human still gates that call inside a forked worker three levels deep; a
[credential bound with `approval: "hold"`](/docs/agent/getting-started/permissions#holding-network-egress-for-approval) still holds the
