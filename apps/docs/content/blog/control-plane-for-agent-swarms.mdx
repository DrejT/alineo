...
[alineod](/docs/guide/swarms) is that control plane, run once, driven over HTTP.
...
[`Alineo.reattach()`](/docs/reference/agent-sdk#alineoreattach), which rebinds to the
...
[`AgentSpec.permissions`](/docs/guide/control/permissions) gate pausing a risky tool
call for a human still gates that call inside a forked worker three levels deep; a
[credential bound with `approval: "hold"`](/docs/guide/control/permissions#holding-network-egress-for-approval) still holds the
...
- [alineod docs](/docs/guide/swarms) — routes, events, budgets, and crash recovery in full.
